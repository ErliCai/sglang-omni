...
const names = ["batch-23-part-1.json", "batch-23-part-2.json", "batch-23-part-3.json"];
...
const batch = (Array.isArray(raw) ? raw : raw.batches).find((x) => x.batchIndex === 23);
...
assert(nodes.length === 138, `节点总数错误: ${nodes.length}`);
assert(ids.size === nodes.length, "存在重复节点 ID");
assert(edges.length === 272, `边总数错误: ${edges.length}`);
...
assert(expectedSymbols.length === 113, `结构符号数错误: ${expectedSymbols.length}`);
