...
if (path.basename(graphPath) !== "batch-3.json") {
  errors.push("输出文件名不是 batch-3.json");
}
...
for (const node of graph.nodes || []) {
  ...
  if (!node.name || !node.summary || !/[\u3400-\u9fff]/u.test(node.summary)) {
    errors.push(`节点缺少中文名称或摘要: ${node.id}`);
  }
  if (!Array.isArray(node.tags) || node.tags.length < 3 || node.tags.length > 5) {
    errors.push(`节点标签数量不合规: ${node.id}`);
  }
  if (!complexities.has(node.complexity)) errors.push(`非法复杂度: ${node.id}`);
  if (fileLevelTypes.has(node.type) && !node.filePath) {
    errors.push(`文件级节点缺少 filePath: ${node.id}`);
  }
  if ((node.type === "function" || node.type === "class") &&
      (!Array.isArray(node.lineRange) || node.lineRange.length !== 2)) {
    errors.push(`函数或类缺少 lineRange: ${node.id}`);
  }
}

for (const batchFile of input.batchFiles) {
  if (!ids.has(`file:${batchFile.path}`)) {
    errors.push(`批次文件缺少节点: ${batchFile.path}`);
  }
}
...
for (const result of extract.results) {
  ...
  for (const fn of result.functions || []) {
    if ((fn.endLine - fn.startLine + 1 >= 10 || exported.has(fn.name)) &&
        !ids.has(`function:${result.path}:${fn.name}`)) {
      errors.push(`缺少显著函数节点: ${result.path}:${fn.name}`);
    }
  }
  for (const cls of result.classes || []) {
    if (((cls.methods || []).length >= 2 ||
         cls.endLine - cls.startLine + 1 >= 20 ||
         exported.has(cls.name)) &&
        !ids.has(`class:${result.path}:${cls.name}`)) {
      errors.push(`缺少显著类节点: ${result.path}:${cls.name}`);
    }
  }
}
...
for (const edge of graph.edges || []) {
  ...
  if (edgeWeights.has(edge.type) && edge.weight !== edgeWeights.get(edge.type)) {
    errors.push(`边权重错误: ${key}`);
  }
}
...
for (const item of expectedImports) {
  if (!actualImports.has(item)) errors.push(`缺少 import 边: ${item}`);
}
for (const item of actualImports) {
  if (!expectedImports.has(item)) errors.push(`多余 import 边: ${item}`);
}

if (graph.nodes.length > 60 || graph.edges.length > 120) {
  errors.push("单文件超过拆分阈值");
}
