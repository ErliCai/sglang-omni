...
const base = "sglang_omni/.ua", names = [1, 2, 3, 4, 5].map((i) => `batch-29-part-${i}.json`);
const actualNames = fs.readdirSync(`${base}/intermediate`).filter((name) => name.startsWith("batch-29")).sort();
...
const batch = (Array.isArray(raw) ? raw : raw.batches).find((x) => x.batchIndex === 29);
...
assert(nodes.length === 252, `节点总数错误: ${nodes.length}`);
assert(ids.size === nodes.length, "存在重复节点 ID");
assert(edges.length === 534, `边总数错误: ${edges.length}`);
...
assert(expectedSymbols.length === 227, `结构符号数错误: ${expectedSymbols.length}`);
