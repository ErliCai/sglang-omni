...
const names = ["batch-19-part-1.json", "batch-19-part-2.json", "batch-19-part-3.json"];
...
const batchesRaw = JSON.parse(fs.readFileSync(`${base}/intermediate/batches.json`, "utf8"));
const batch = (Array.isArray(batchesRaw) ? batchesRaw : batchesRaw.batches).find((x) => x.batchIndex === 19);
...
assert(nodes.length === 134, `节点总数错误: ${nodes.length}`);
assert(ids.size === nodes.length, "存在重复节点 ID");
assert(edges.length === 253, `边总数错误: ${edges.length}`);

const nodeTypes = new Set(["file", "document", "function", "class"]);
...
assert(expectedSymbols.length === 109, `结构符号数错误: ${expectedSymbols.length}`);

const weights = { contains: 1, imports: 0.7, calls: 0.8, exports: 0.8, depends_on: 0.6 };
...
for (let i = 0; i < parts.length; i++) {
  ...
  assert(parts[i].nodes.length <= 60, `分片 ${i + 1} 节点超限`);
  assert(parts[i].edges.length <= 120, `分片 ${i + 1} 边超限`);
}
