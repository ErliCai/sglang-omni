...
const names = ["batch-20-part-1.json", "batch-20-part-2.json", "batch-20-part-3.json"];
...
const batch = (Array.isArray(raw) ? raw : raw.batches).find((x) => x.batchIndex === 20);
...
assert(nodes.length === 127, `节点总数错误: ${nodes.length}`);
assert(ids.size === nodes.length, "存在重复节点 ID");
assert(edges.length === 250, `边总数错误: ${edges.length}`);
...
assert(expectedSymbols.length === 102, `结构符号数错误: ${expectedSymbols.length}`);
