...
const base = "sglang_omni/.ua", names = [1, 2, 3].map((i) => `batch-27-part-${i}.json`);
const actualNames = fs.readdirSync(`${base}/intermediate`).filter((name) => name.startsWith("batch-27")).sort();
...
const batch = (Array.isArray(raw) ? raw : raw.batches).find((x) => x.batchIndex === 27);
...
assert(nodes.length === 126, `节点总数错误: ${nodes.length}`);
assert(ids.size === nodes.length, "存在重复节点 ID");
assert(edges.length === 259, `边总数错误: ${edges.length}`);
...
assert(expectedSymbols.length === 101, `结构符号数错误: ${expectedSymbols.length}`);

const weights = { contains: 1, imports: 0.7, calls: 0.8, exports: 0.8, depends_on: 0.6, inherits: 0.9 }, edgeKeys = new Set();
