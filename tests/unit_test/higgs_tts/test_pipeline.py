...
@pytest.mark.parametrize("dtype", ["float32", "float16", "auto"])
def test_mlx_rejects_unsupported_dtype_before_infrastructure(monkeypatch, dtype):
    from sglang.srt.utils import tensor_bridge

    monkeypatch.setattr(tensor_bridge, "use_mlx", lambda: True)
    instance = builder()
    options = merged(instance, EngineArgs(dtype=dtype).overrides())
    with pytest.raises(ValueError, match="Higgs MLX.*dtype"):
        instance.adjust_overrides(options)
    args = SimpleNamespace(
        **instance._mps_requirements(),
        dtype=dtype,
        cuda_graph_config=CudaGraphConfig.from_dict(
...
@pytest.mark.parametrize("dtype", ["bfloat16", "bf16"])
def test_mlx_accepts_bfloat16_dtype(monkeypatch, dtype):
    from sglang.srt.utils import tensor_bridge

    monkeypatch.setattr(tensor_bridge, "use_mlx", lambda: True)
    instance = builder()
    instance.adjust_overrides(merged(instance, {"dtype": dtype}))
    instance.validate_before_infrastructure(
        SimpleNamespace(
            **instance._mps_requirements(),
            dtype="bfloat16",
            cuda_graph_config=CudaGraphConfig.from_dict(
...
def test_torch_mps_dtype_override_is_not_restricted_by_mlx_guard(monkeypatch):
    from sglang.srt.utils import tensor_bridge

    monkeypatch.setattr(tensor_bridge, "use_mlx", lambda: False)
    instance = builder()
    instance.adjust_overrides(merged(instance, {"dtype": "float32"}))
